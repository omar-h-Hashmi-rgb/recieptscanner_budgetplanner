...
const addTransaction = async (req, res) => {
  const { name, category, cost, addedOn, isIncome, note, tags = [] } = req.body;

  try { 
    let finalCategory = category;

    // Auto-categorization: If no category provided or it's a generic one, try to find a matching rule
    if (!category || category === 'Miscellaneous' || category === '') {
      if (name && !isIncome) { // Only apply auto-categorization for expenses
        const rules = await CategoryRule.find({ user: req.user.id });
        for (const rule of rules) {
          if (name.toLowerCase().includes(rule.keyword)) {
    ...
    const transaction = new IncomeExpense({
      user: req.user.id,
      name,
      category: finalCategory || category || 'Miscellaneous',
      cost,
      addedOn,
      isIncome,
      note,
...
const getTransactions = async (req, res) => {
  try {
    const { search, isIncome, category, startDate, endDate, tags, page = 1, limit = 10 } = req.query;
    ...
    if (tags) {
      // Support filtering by multiple tags (comma-separated)
...
const updateTransaction = async (req, res) => {
  try {
    ...
    transaction.tags = Array.isArray(tags) ? tags.filter(tag => tag && tag.trim()) : transaction.tags;
...
// @desc    Get all categories (both expense and income)
// @route   GET /api/transactions/categories
// @access  Private
const getAllCategories = async (req, res) => {
  try {
    ...
    const defaultExpenseCategories = [
      ...
      'Entertainment', 'Groceries', 'Miscellaneous'
    ];
    
    const defaultIncomeCategories = [
      'Salary', 'Freelance / Side Gig', 'Investment Returns', 'Gifts', 'Refunds'
    ];

    // Get user's custom categories from database
    const userExpenseCategories = await IncomeExpense.distinct('category', { 
      user: req.user._id, 
      isIncome: false 
    });
    ...
    const expenseCategories = [...new Set([...defaultExpenseCategories, ...userExpenseCategories])].sort();
    const incomeCategories = [...new Set([...defaultIncomeCategories, ...userIncomeCategories])].sort();
    ...
    res.status(200).json({
      ...
      data: {
        ...
        all: [...expenseCategories, ...incomeCategories].sort()
      }
    });
...
const exportTransactions = async (req, res) => {
  try {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="receiptwise_transactions.csv"');
...
const getAllTags = async (req, res) => {
  try {
    const tags = await IncomeExpense.aggregate([
